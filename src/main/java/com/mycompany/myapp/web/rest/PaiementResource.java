...
public class PaiementResource {
        ...
        Optional<Paiement> result = paiementRepository
            ...
            .map(existingPaiement -> {
                if (paiement.getReference() != null) {
                    existingPaiement.setReference(paiement.getReference());
                }
    ...
    public ResponseEntity<List<Paiement>> getAllPaiements(@org.springdoc.api.annotations.ParameterObject Pageable pageable) {
        log.debug("REST request to get a page of Paiements");
        Page<Paiement> page = paiementRepository.findAll(pageable);

...
import './document.css'; // Import custom CSS
import './Invoice.css'
      ...
      <div>
        ...
        <div className="table-wrapper">
          <div className="d-flex justify-content-between align-items-center mb-3 p-3 custom-bg-color text-white rounded">
            <h2 id="document-heading" data-cy="DocumentHeading" className="mb-0">
              <Translate contentKey="faeApp.document.home.title">Documents</Translate>
            </h2>
            <div className="d-flex justify-content-end ajust1" style={{ gap: '10px' }}>
              <Button className="bt btn-info me-2" onClick={handleSyncList} disabled={loading} style={{ width: '230px', height: '48px' }}>
                <FontAwesomeIcon icon="sync" spin={loading} />{' '}
                <Translate contentKey="faeApp.article.home.refreshListLabel">Refresh List</Translate>
              </Button>
              <Button className="btn btn-success " onClick={toggleModal} style={{ width: '230px', height: '48px' }}>
                <FontAwesomeIcon icon="plus" />
                &nbsp;
                <Translate contentKey="faeApp.document.home.createLabel">Ajouter une facture</Translate>
              </Button>
            </div>
          </div>
                  ...
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>
                      {document.dateCreation ? (
                        <TextFormat type="date" value={document.dateCreation} format={APP_LOCAL_DATE_FORMAT} />
                      ) : null}
                    </td>
                    <td>
          ...
          <Modal isOpen={showModal} toggle={toggleModal} className="custom-modal">
            ...
            <ModalBody>
                  ...
                  {uniqueClients.map((client, index) => (
                    client.clients && (
                      <option key={`${client.clients.id}-${client.clients.nom}`} value={client.clients.id}>
                        {
                          clientList.find(clien => clien.user && clien.id == client.clients.id ) && clientList.find(clien => clien.user && clien.id == client.clients.id).user
                            ? clientList.find(clien => clien.user && clien.id == client.clients.id).user.firstName
                            : ''
                        }
                      </option>
                    )
              ...
              {selectedClient !== null && clientBordereauList.find(client => client.clients && client.clients.id === selectedClient) && (
                <div className="table-responsive" style={{width:'700px'}}>
              ...
              {selectedClient && (
                <div className="d-flex align-items-center mb-3" style={{ cursor: 'pointer' }} onClick={toggleAdditionalFields}>
                  <span style={{ color: 'lightblue', margin: 'auto' }}>Ajouter d'autres informations <FontAwesomeIcon icon={showAdditionalFields ? faChevronUp : faChevronDown}></FontAwesomeIcon></span>
                </div>
              )}
        ...
        <div className="table-wrapper">
          <div className="d-flex justify-content-between align-items-center mb-3 p-3 custom-bg-color text-white rounded">
            ...
            <div className="d-flex justify-content-end " style={{ gap: '10px' }}>
              <Button className="btn btn-info" onClick={handleSyncList} disabled={loading} style={{ width: '220px', height: '40px' ,marginLeft:'200px'}}>
                <FontAwesomeIcon icon="sync" spin={loading} />{' '}
                <Translate contentKey="faeApp.document.home.refreshListLabel">Actualiser la liste</Translate>
              </Button>
            </div>
          </div>
          <div className="table-responsive">
            {userDocuments && userDocuments.length > 0 ? (
              <Table className="table-striped custom-table">
                ...
                <tr>
                  ...
                  <th className="hand" onClick={sort('reference')}>
                    <Translate contentKey="faeApp.document.reference">Référence</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('montantTotal')}>
                    <Translate contentKey="faeApp.document.montantTotal">Montant Total</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  ...
                  <th className="hand" onClick={sort('dateCreation')}>
                    <Translate contentKey="faeApp.document.dateCreation">Date de Création</Translate> <FontAwesomeIcon icon="sort" />
                  </th>
                  <th className="hand" onClick={sort('dateModification')}>
                    Date de Livraison
                  </th>
                ...
                <tbody>
                {userDocuments
                  .filter(f => f.id && factureList.find(d => d.document && f.id === d.document.id))
                  .map((document, i) => {
                    ...
                    return (
                      <tr key={`entity-${i}`} data-cy="entityTable">
                        ...
                        <td>{document.reference}</td>
                        <td>{document.montantTotal}</td>
                        <td>{facture ? <div>{facture.etat}</div> : <div>Aucune facture</div>}</td>
                        <td>
                          {document.dateCreation ? (
                            <TextFormat type="date" value={document.dateCreation} format={APP_LOCAL_DATE_FORMAT} />
                          ) : null}
                        ...
                        <td className="text-right">
                          {facture && facture.etat === 'Non payé' && (
                            <Button color="primary" size="sm" className="btn-small" data-cy="entityEditButton" onClick={() => handlePayment(facture, document)} style={{ width: '100px' }}>
                              <FontAwesomeIcon icon={faCreditCard} /> <span className="d-none d-md-inline">Payer</span>
                            </Button>
                          )}
                          {facture && facture.etat !== 'Non payé' && (
                            <div >
                              <Button color="primary" size="sm" data-cy="entityViewButton" onClick={togglle} className="btn-small" style={{ width: '100px' }}>
                                <FontAwesomeIcon icon={faCreditCard} /> <span className="d-none d-md-inline">imprimer</span>
                              </Button>

                              <Button color="warning" size="sm" data-cy="entityViewButton" onClick={() => handleOtherAction(document)} className="btn-small" style={{ width: '100px' }}>
                                <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">Demande R</span>
                              </Button>
                            </div>
                          )}
                        </td>
          ...
          {totalItems ? (
            <div className={userDocuments && userDocuments.length > 0 ? '' : 'd-none'}>
          ...
          <Modal isOpen={modal} toggle={toggle} className="custom-modal-width">
            <ModalHeader toggle={toggle}>Procéder au paiement</ModalHeader>
            <ModalBody>{selectedFacture && selectedDoc && <PaiementUpdate factureId={selectedFacture.id} documentId={selectedDoc.id} />}</ModalBody>
            <ModalFooter>
              <Button color="secondary" onClick={toggle} className="butt">Annuler</Button>
            </ModalFooter>
          </Modal>
          <Modal isOpen={modall} toggle={togglle} size="lg" className="custom-modal">
                ...
                <div id="invoice-container" className="invoice-container">
                  ...
                  <div className="details">
                    ...
                    <div className="facture-num">
                      <p>Numéro de facture: ITA12367</p>
                    </div>
          ...
          <Modal isOpen={secondModal} toggle={toggleSecondModal} className="custom-modal" >
            <ModalHeader toggle={toggleSecondModal}>Détails de la facture</ModalHeader>
            <ModalBody className="custom">{selectDocDR && <DemandeRembourssementUpdate documentId={selectDocDR.id} />}</ModalBody>
            <ModalFooter>
